...
describe('email unit test', () => {
  it('create', () => {
    ...
    const email = EmailNotificationConfig.create('routine-id', 'some-name', 'some@bar.com', curDate);
  ...
  it('fail with non-email', () => {
    const curDate = DateTime.fromISO('2018-01-01T00:00:00.000Z').toJSDate();
    expect(() => EmailNotificationConfig.create('routine-id', 'some-name', 'not-email', curDate)).to.throw('email must be an email');
  });
});
