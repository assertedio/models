...
describe('slackWebhook unit test', () => {
  it('create', () => {
    ...
    const slackWebhook = SlackWebhookNotificationConfig.create('routine-id', 'some-name', 'https://hooks.slack.com/thing', curDate);
  ...
  it('fail with non-url', () => {
    const curDate = DateTime.fromISO('2018-01-01T00:00:00.000Z').toJSDate();
    expect(() => SlackWebhookNotificationConfig.create('routine-id', 'some-name', 'not-url', curDate)).to.throw(
  ...
  it('fail with non-slackWebhook', () => {
    const curDate = DateTime.fromISO('2018-01-01T00:00:00.000Z').toJSDate();
    expect(() => SlackWebhookNotificationConfig.create('routine-id', 'some-name', 'https://hooks.slack.com=====', curDate)).to.throw(
