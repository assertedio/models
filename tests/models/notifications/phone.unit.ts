...
describe('phone unit test', () => {
  it('create', () => {
    ...
    const phone = PhoneNotificationConfig.create('routine-id', 'project-id', 'some-name', '+7 800 555 35 35', PHONE_NOTIFY_TYPE.SMS, curDate);
  ...
  it('fail with non-phone', () => {
    const curDate = DateTime.fromISO('2018-01-01T00:00:00.000Z').toJSDate();
    expect(() => PhoneNotificationConfig.create('routine-id', 'project-id', 'some-name', 'not-a-phone', PHONE_NOTIFY_TYPE.SMS, curDate)).to.throw(
